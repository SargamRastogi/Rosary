...
style="background-image: url('/images/slide-3.jpg');">
    ...
    <div class="relative z-10 flex flex-col justify-center items-center text-center h-full text-white px-6">
        <h1 class="text-4xl md:text-5xl font-bold">Retail Interior <span class="text-brand-gold">Solutions</span> </h1>
        <p class="mt-4 max-w-2xl text-lg">
            Designing modern retail showrooms & creative decor spaces that attract and engage customers.
        </p>
    </div>
</section>
    ...
    <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center px-4 sm:px-6">
        ...
        <!-- LEFT TEXT -->
         <div class="text-center md:text-left flex flex-col items-center md:items-start">
            <h3 class="text-4xl font-bold text-neutral-900 mb-4">Showroom <span class="text-brand-gold">Interior</span> </h3>

            <p class="text-neutral-600 text-sm sm:text-base md:text-lg leading-relaxed mb-4">
                Stylish, customer-focused showroom layouts designed to highlight your products  
                and create an immersive buying experience.
            </p>

            <p class="text-neutral-600 text-sm sm:text-base md:text-lg leading-relaxed mb-4">
                Whether it's fashion, furniture, electronics, or luxury goods—our showroom  
                designs ensure a balance of aesthetic appeal and functional flow.
            </p>

            <a href="{{ route('projects.showroom') }}" class="inline-block px-6 py-3 mt-3 rounded-lg border border-[#cea56c] text-[#cea56c] hover:bg-[#cea56c] hover:text-white transition">
                View More →
            </a>
        </div>
        ...
        <div class="relative w-full overflow-hidden rounded-xl border-4 border-[#cea56c] shadow-xl">
            ...
            <div class="showroomTrack flex transition-transform duration-700 ease-in-out">
                <img src="/images/slide-3.jpg" class="w-full h-[380px] object-cover flex-shrink-0" />
                <img src="/images/retail-s2.jpg" class="w-full h-[380px] object-cover flex-shrink-0" />
                <img src="/images/retail-s3.jpg" class="w-full h-[380px] object-cover flex-shrink-0" />
            </div>

            <div class="showroomDots absolute bottom-4 w-full flex justify-center gap-3">
...
<section class="py-12 md:py-16 bg-[#f9f7f4]">
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 items-center px-4 sm:px-6">
        ...
        <!-- TEXT (First in Mobile) -->
        <div class="order-1 md:order-2 text-center md:text-left">
            <h3 class="text-4xl font-bold text-neutral-900 mb-4">
                Home Decor & <span class="text-brand-gold">Display</span>
            </h3>

            <p class="text-neutral-600 text-sm sm:text-base md:text-lg leading-relaxed mb-4">
                Elegant decor display solutions to showcase products beautifully—lighting,  
                shelving, color themes, and arrangement made to convert interest into sales.
            </p>

            <p class="text-neutral-600 text-sm sm:text-base md:text-lg leading-relaxed mb-4">
                Ideal for lifestyle stores, gift shops, craft outlets, and luxury decor boutiques.
            </p>
        ...
        <!-- IMAGE (Second in Mobile) -->
        <div class="order-2 md:order-1 relative w-full overflow-hidden rounded-xl border-4 border-[#cea56c] shadow-xl">

            <div class="homeDecorTrack flex transition-transform duration-700 ease-in-out">
                <div class="w-full flex-shrink-0">
                    <img src="/images/decor-p1.jpg" class="w-full h-[380px] object-cover" />
                </div>

                <div class="w-full flex-shrink-0">
                    <img src="/images/decor-p2.jpg" class="w-full h-[380px] object-cover" />
                </div>

                <div class="w-full flex-shrink-0">
                    <img src="/images/decor-p3.jpg" class="w-full h-[380px] object-cover" />
                </div>
            </div>

            <div class="homeDecorDots absolute bottom-4 w-full flex justify-center gap-3">
                <span class="dot w-3 h-3 bg-white border border-[#cea56c] rounded-full cursor-pointer"></span>
                <span class="dot w-3 h-3 bg-white border border-[#cea56c] rounded-full cursor-pointer"></span>
                <span class="dot w-3 h-3 bg-white border border-[#cea56c] rounded-full cursor-pointer"></span>
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // SHOWROOM SLIDER
    let sIndex = 0;
    const sTrack = document.querySelector(".showroomTrack");
    const sDots = document.querySelectorAll(".showroomDots .dot");
    const sCount = sDots.length;

    function showroomSlide(n) {
        sIndex = n;
        sTrack.style.transform = `translateX(-${sIndex * 100}%)`;
        sDots.forEach(d => d.classList.remove("bg-[#cea56c]"));
        sDots[sIndex].classList.add("bg-[#cea56c]");
    }

    sDots.forEach((dot, i) => dot.addEventListener("click", () => showroomSlide(i)));

    setInterval(() => {
        sIndex = (sIndex + 1) % sCount;
        showroomSlide(sIndex);
    }, 3000);
...
// DECOR SLIDER
let dIndex = 0;
const dTrack = document.querySelector(".homeDecorTrack");
const dDots = document.querySelectorAll(".homeDecorDots .dot");
const dCount = dDots.length;

function decorSlide(n) {
    dIndex = n;
    dTrack.style.transform = `translateX(-${dIndex * 100}%)`;
    dDots.forEach(d => d.classList.remove("bg-[#cea56c]"));
    dDots[dIndex].classList.add("bg-[#cea56c]");
}

dDots.forEach((dot, i) => dot.addEventListener("click", () => decorSlide(i)));

setInterval(() => {
    dIndex = (dIndex + 1) % dCount;
    decorSlide(dIndex);
}, 3000);
