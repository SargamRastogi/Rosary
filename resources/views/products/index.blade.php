    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div id="subcategoryWrapper" class="flex gap-6 sm:gap-10 overflow-x-auto md:justify-center pb-4 no-scrollbar pt-5 px-2">
            @foreach($subcategories as $index => $sub)
                <div onclick="loadProducts({{ $sub->id }}, '{{ $sub->name }}', this)"
                    class="subcategory-card cursor-pointer flex flex-col items-center {{ $index === 0 ? 'active-sub':'' }}">
                    
                    <div class="subcategory-img">
                        <img src="{{ asset('storage/'.$sub->image_path) }}" class="w-full h-full object-cover rounded-full">
                    </div>

                    <p class="subcategory-label mt-4 text-center text-sm font-medium whitespace-nowrap {{ $index === 0 ? 'active-underline':'' }}">{{ $sub->name }}</p>
                </div>
            @endforeach
        </div>
...
<div class="group bg-white rounded-2xl shadow 
    ...
    <a href="{{ route('product.show', $product->slug) }}"
       class="relative overflow-hidden bg-gray-100">
        ...
        @php $discount = rand(10,45); @endphp
