    ...
    <div class="gallery-wrapper mb-8 md:mb-0">
        ...
        <div class="block md:hidden overflow-x-auto whitespace-nowrap pb-4 -mx-4 px-4 scroll-smooth" id="mobileGallery">
            ...
            <div class="flex justify-center mt-4 gap-2">
                @foreach($allImages as $index => $img)
                    <button class="mobile-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-[#BF955C]' : 'bg-gray-300' }}" 
                            data-index="{{ $index }}"
        ...
        <div class="hidden md:block">
            <div class="grid grid-cols-2 gap-4 max-w-[650px]" id="galleryGrid">
                @foreach($allImages as $index => $img)
                    ...
                    <div 
                        class="gallery-cell {{ $isFirst ? 'col-span-2' : '' }} 
                    ...
                    >
                        <img 
                            ...
                            class="gallery-grid-img w-full 
                                   {{ $isFirst ? 'h-[500px] lg:h-[700px]' : 'h-[250px] lg:h-[350px]' }} 
                                   object-cover 
    ...
    <button id="wishlistBtn"
        ...
        <div class="mt-10 sm:mt-14">
            ...
            <div class="mt-4 text-neutral-700 space-y-2 text-sm sm:text-base">
                ...
                @endif
                @if($product->pack_content)
                    <p><strong>Pack Content:</strong> {{ $product->pack_content }}</p>
                @endif
                @if($product->brand)
...
{{-- ================= MODAL WITH IMAGE ZOOM (HOVER ZOOM) ================= --}}
<div id="imageModal"
    ...
    <div class="flex flex-col sm:flex-row w-full max-w-7xl px-4 sm:px-10 gap-4 sm:gap-8">
        ...
        <div class="hidden sm:flex flex-col gap-4 max-h-[80vh] overflow-y-auto pr-2">
            @foreach($allImages as $index => $img)
                ...
                <img
                    src="{{ asset('storage/' . $path) }}"
                    class="modal-thumb w-16 lg:w-20 h-20 lg:h-24 object-cover rounded-md cursor-pointer border-2 transition-all duration-200 {{ $index === 0 ? 'border-[#BF955C]' : 'border-transparent hover:border-white/50' }}"
                    data-index="{{ $index }}"
        ...
        {{-- MOBILE DOTS IN MODAL --}}
        <div class="flex sm:hidden justify-center mt-4 gap-2">
            @foreach($allImages as $index => $img)
                <button class="modal-mobile-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-gray-500' }}" 
                        data-index="{{ $index }}"
                        onclick="changeModalImage({{ $index }})"></button>
            @endforeach
        </div>
        ...
        <div class="flex sm:hidden gap-2 overflow-x-auto pb-2 mt-2 px-1">
            @foreach($allImages as $index => $img)
                ...
                <img
                    src="{{ asset('storage/' . $path) }}"
                    class="modal-mobile-thumb w-16 h-16 object-cover rounded-md cursor-pointer border-2 flex-shrink-0 {{ $index === 0 ? 'border-[#BF955C]' : 'border-transparent' }}"
                    data-index="{{ $index }}"
                    onclick="changeModalImage({{ $index }})"
                    loading="lazy"
                >
